...
class ATTLSTM(nn.Module):
    def __init__(self,pretrain_embedding_weight):
        super(ATTLSTM, self).__init__()
        self.vocab_size=constant.vocab_size
        self.label_size=constant.label_size
        self.embedding_size=constant.glove_dim
        self.hidden_size=constant.lstm_hidden_dim
        self.pretrain_embedding_weight=pretrain_embedding_weight

        self.dropout=constant.dropout
        self.emb_dropout = constant.emb_dropout
        self.rnn_dropout = constant.rnn_dropout
        self.batch_size=constant.batch_size

        self.embed=nn.Embedding(self.vocab_size,self.embedding_size)
        self.embed.weight.data.copy_(torch.from_numpy(self.pretrain_embedding_weight))
        ...
        self.lstm=nn.LSTM(input_size=self.embedding_size,hidden_size=self.hidden_size,bidirectional=True,batch_first=True,num_layers=1,dropout=self.rnn_dropout)
    ...
    def forward(self,inputs,e1s,e1e,e2s,e2e,p1,p2):
        ...
        #(B,S,E)
        # print(embedding)
        embedding_drop=self.embed_drop(word_embed)
