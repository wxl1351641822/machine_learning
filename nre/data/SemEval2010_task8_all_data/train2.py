...
from ATTCNN_nonline_lextual import ATTCNN as MODEL
from ATTCNN_nonline_lextual import DistanceLoss
...
lr=constant.lr
loss_func=DistanceLoss(label_size)
max_acc=torch.tensor(0.0)
all_y = torch.zeros(label_size, label_size)
...
for epoch in range(constant.num_epoch):
    ...
    for j in range(9):
        ...
        # for i,(x,y) in tqdm(enumerate(zip(train_data['token_id'],train_data['relation_id']))):
            wo,WL=model(torch.tensor(x),e1s,e1e,e2s,e2e,pos_1,pos_2)
            # predict=model.predict(wo,WL)
            y=torch.tensor(y)
            loss=loss_func(wo,WL,y,all_y)
            # print(pos_1)
            # print(pos_2)
        ...
        predict=model.predict(wo,WL,all_y)
        # print(predict)
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(len(test_data['token_id'])//batch_size)):
            ...
            # print(pred)
            # soft=torch.log_softmax(pred,dim=1)
            predict[i * batch_size:(i + 1) * batch_size]=model.predict(wo,WL,all_y)
        s1=''
        s2=''
